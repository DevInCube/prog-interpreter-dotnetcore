namespace Prog
{
    ...
    {
        private readonly Stack<HashSet<Variable>> _scopes = new();

        public void EnterScope() => _scopes.Push([]);

        public void LeaveScope() => _scopes.Pop();

        public Variable? FindSymbol(string symbol) => _scopes
            .FirstOrDefault(x => x.Any(t => t.Name == symbol))
            ?.FirstOrDefault(t => t.Name == symbol);
